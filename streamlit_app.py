...
""", unsafe_allow_html=True)
            ...
            # 2. Charts (Probabilities and Features)
            ...
            st.markdown("</div>", unsafe_allow_html=True) # close chart grid

            # 3. Keywords & Models / Risks & Steps
            st.markdown("<div style='display:grid;grid-template-columns:1fr 1fr;gap:18px;margin-top:14px'>", unsafe_allow_html=True)
            
            # Left Column: Keywords & Models
            st.markdown("<div class='result-card'>", unsafe_allow_html=True)
            st.markdown("**Extracted keywords**") # Use Markdown Bolding
            ...
            st.markdown("**Suggested business model(s)**") # Use Markdown Bolding
            ...
            # Right Column: risks, steps
            st.markdown("<div class='result-card'>", unsafe_allow_html=True)
            st.markdown("**Key risks**") # Use Markdown Bolding
            for r in risks:
                st.markdown(f"- {r}")
            st.markdown("<hr style='margin-top:12px;margin-bottom:12px'/>", unsafe_allow_html=True)
            st.markdown("**Recommended next steps**") # Use Markdown Bolding
            for i, s in enumerate(steps, 1):
                st.markdown(f"{i}. {s}")
            st.markdown("</div>", unsafe_allow_html=True) # close right column
            
            st.markdown("</div>", unsafe_allow_html=True) # close second grid

            # 4. Quick Research Links
            st.markdown("<div class='result-card' style='display:flex;flex-direction:column;gap:8px'>", unsafe_allow_html=True)
            st.markdown("Quick research links", unsafe_allow_html=True)
